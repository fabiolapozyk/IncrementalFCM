...
class ValidityMeasures(object):
    ...
    input: 
        true_label: array of integers corresponding to correct label of data point
        clusters: array of integers corresponding to cluster which a data point belongs 
    '''
    def getConfusionMatrix(true_label, clusters,c):
            M = clusters.shape[0]
            C =c
            n_label = np.unique(true_label).shape[0]
